<?php /*a:1:{s:67:"D:\phpstudy_pro\WWW\ftpuser\thinkadmin\app\admin\view\api\upload.js";i:1728976221;}*/ ?>
define(['md5', 'notify'], function (SparkMD5, Notify, allowMime) {
    allowMime = JSON.parse('<?php echo $exts; ?>');
    ...
    function UploadAdapter(elem, done) {
        return new (function (elem, done) {
            ...
            this.adapter = new Adapter(this.option, layui.upload.render({
                url: '<?php echo url("admin/api.upload/file",[],false,true); ?>', auto: false, elem: elem, accept: 'file', multiple: this.option.mult, exts: this.option.exts.join('|'), acceptMime: this.option.mimes.join(','), choose: function (obj) {
    ...
    Adapter.prototype.upload = function (files, done) {
        ...
        layui.each(files, function (index, file) {
            ...
            if (that.option.size && file.size > that.option.size) {
                that.event('upload.error', {file: file}, file, '<?php echo lang("大小超出限制！"); ?>');
            }
        });
        layui.each(files, function (index, file) {
            // 禁传异常状态文件
                ...
                require(['compressor'], function (Compressor) {
                    ...
                    new Compressor(file, Object.assign(options, {
                        success(blob) {
                            ...
                            that.event('upload.error', {file: file}, file, '<?php echo lang("图片压缩失败！"); ?>');
                        }
                    }));
                });
            } else {
    ...
    Adapter.prototype.request = function (file, done) {
        ...
        jQuery.ajax("<?php echo url('admin/api.upload/state',[],false,true); ?>", {
                ...
                if (parseInt(ret.code) === 404) {
                    ...
                    uploader.form.append('file', file, file.name), jQuery.ajax({
                        xhrFields: {withCredentials: ret.data.uptype === 'local'}, headers: uploader.head, url: uploader.url, data: uploader.form, type: uploader.type || 'post', xhr: function (xhr) {
                            ...
                            that.event('upload.error', {file: file}, file, '<?php echo lang("上传接口异常！"); ?>');
                            ...
                            if (typeof ret !== 'object') {
                                ret = {code: 1, url: file.xurl, info: '<?php echo lang("文件上传成功！"); ?>'};
                            }
                            /*! 检查单个文件上传返回的结果 */
                            if (typeof ret === 'object' && ret.code < 1) {
                                that.event('upload.error', {file: file}, file, ret.info || '<?php echo lang("文件上传失败！"); ?>');
                            } else if (uploader.uptype === 'alist' && parseInt(ret.code) !== 200) {
                                that.event('upload.error', {file: file}, file, ret.message || '<?php echo lang("文件上传失败！"); ?>');
                            } else {
                                that.done(ret, file.index, file, done, '<?php echo lang("文件上传成功！"); ?>');
                            }
                        }
                    });
                } else if (parseInt(ret.code) === 200) {
                    (file.xurl = ret.data.url), that.progress('100.00', file);
                    that.done({code: 1, url: file.xurl, info: file.xstats, data: {code: 200, url: file.xurl}}, file.index, file, done, '<?php echo lang("文件秒传成功！"); ?>');
                } else {
                    that.event('upload.error', {file: file}, file, ret.info || ret.error.message || '<?php echo lang("文件上传出错！"); ?>');
                }
            }
        });
    };
    ...
    Adapter.prototype.done = function (ret, idx, file, done, message) {
        /*! 检查单个文件上传返回的结果 */
        if (ret.code < 1) return $.msg.tips(ret.info || '<?php echo lang("文件上传失败！"); ?>');
        if (typeof file.xurl !== 'string') return $.msg.tips('<?php echo lang("无效的文件上传对象！"); ?>');
        jQuery.post("<?php echo url('admin/api.upload/done',[],false,true); ?>", {id: file.id, hash: file.xmd5});
        /*! 单个文件上传成功结果处理 */
    ...
    Adapter.prototype.hash = function (file) {
        ...
        /*! 兼容不能计算文件 HASH 的情况 */
        let IsDate = '<?php echo htmlentities((string) (isset($nameType) && ($nameType !== '')?$nameType:"")); ?>'.indexOf('date') > -1;
    ...
    function NotifyExtend(file) {
        let that = this, message = "<?php echo lang('上传进度 %s', ['<span data-upload-progress>0%</span>']); ?>";
        ...
        this.setProgress = function (number) {
            ...
            this.$elem.find('.growl-notification__desc').html(message || '<?php echo lang("文件上传失败！"); ?>');
            ...
            this.$elem.find('.growl-notification__desc').html(message || '<?php echo lang("文件上传成功！"); ?>');
