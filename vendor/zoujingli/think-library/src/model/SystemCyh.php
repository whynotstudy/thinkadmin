...
class SystemCyh extends Model
{
     ...
     * @var string
     */
    protected $oplogName = '测试增加';

    /**
     * 日志类型
     * @var string
     */
    protected $oplogType = '测试增加菜单';
    ...
    {
        $map = ['how' => $type, 'status' => 1, 'deleted' => 0];
        $bases = static::mk()->where($map)->order('sort desc,id asc')->column('how,mid,name,content', 'how');
    ...
    {
        $types = static::mk()->where(['deleted' => 0])->distinct()->column('how');
        if (empty($types) && empty($simple)) $types = ['默认渠道'];
        return $types;
    }
